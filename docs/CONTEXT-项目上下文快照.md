...
- **当前状态：P0/P1/P2 全部完成并通过验收；代码审查 12 项稳定化修复全部完成；扫描配对修复（跨目录同名，11,009→12,591）已合并；审查后新增：主题切换（默认浅色）/ 日期面板方案 C / 默认 8 列 / 详情返回保留滚动位置（KeepAlive）；Git 私有仓库 + GitHub 远程（isChen0111/icloud-app，SSH 推送）版本管理。**
